      ...
      void processMouse(retro_input_state_t aCallback, int device, float gampad_cursor_speed, bool analog_response_is_cubic, int analog_deadzone, float mouse_speed)
      {
         ...
         static const Common::EventType eventID[2][2] =
         {
			...
			static const unsigned gampad_key_map[8][3] = {
				{ RETRO_DEVICE_ID_JOYPAD_X,      (unsigned)Common::KEYCODE_F1,    (unsigned)Common::ASCII_F1    }, // F1
				...
				{ RETRO_DEVICE_ID_JOYPAD_START,  (unsigned)Common::KEYCODE_ESCAPE,    (unsigned)Common::ASCII_ESCAPE        }, // Escape
			};
	    ...
	    _ptrmouseButton=0;
         ...
         if(down != _joypadmouseButtons[1])
         {
			...
			for(int i = 0; i < 8; i ++)
			{
				...
				down |=
					(key == RETRO_DEVICE_ID_JOYPAD_START  && touchKeyCode == RETRO_DEVICE_ID_JOYPAD_START) ||
